...
class IrActionsReport(models.Model):
    ...
    @staticmethod
    def remove_non_printable_chars(byte_string):
        string = byte_string.decode('utf-8')
        cleaned_string = ''.join(ch for ch in string if ch.isprintable())
        return cleaned_string.encode('utf-8')

    @staticmethod
    def sanitize_xml_string(xml):
        return xml
    @staticmethod
        ...
        with ZipFile(mem_zip, 'w', ZIP_DEFLATED) as output:
            for zi in document.zip.filelist:
                if zi in document.parts:
                    xml = etree.tostring(document.parts[zi].getroot(),encoding='utf-8')
                    sanitized_xml = self.sanitize_xml_string(self.remove_non_printable_chars(xml))
                    output.writestr(zi.filename, sanitized_xml)
    ...
    def __footer_qrcode(self, block, record, fields):
        ...
        if matched_field and not hasattr(record, matched_field[0]):
            ...
            field = matched_field
        data = f'http://localhost:8069/signature/{self.model}/{record.id}'
        config_parameter = self.env['ir.config_parameter'].sudo().search_read([('key', '=', 'web.base.url')], ['value'])
        if config_parameter:
            data = config_parameter[0]['value'] + f'/signature/{self.model}/{record.id}'
        ...
        with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
            ...
            if isinstance(block, _Cell):
                block.text = ''
                block.add_paragraph().add_run().add_picture(tmp_file, width=Inches(2.0))
            else:
                block.text = ''
                block.add_run().add_picture(tmp_file, width=Inches(2.0))
    ...
    def _match_and_replace(self, block, record, fields):
        ...
        if '{qr_signature}' in block.text:
            self.__footer_qrcode(block, record, fields)
